...
async function clearUnica() {
  ...
  process.title = 'BloodHound | Limpar com DM única';
  ...
  for (const [index, msg] of msgs.entries()) {
    ...
    await msg.delete().then(async () => {
      contador++;
      process.title = `BloodHound | Limpar com DM única | ${contador}/${msgs.length} mensagens apagadas`;
...
async function clearAbertas() {
  ...
  process.title = 'BloodHound | Limpar com DMs abertas';
  const config = require('./config.json');
  ...
  for (const dm of dms) {
    contador++;
    process.title = `BloodHound | Limpar com DMs abertas | ${contador}/${dms.length} DMs limpas`;
    let contador_msgs = 0;
  ...
  for (const entry of zipEntries) {
    ...
    for (const recipientId of pegarRecipients(channelData.recipients)) {
      ...
      if (dmChannel) {
        ...
        contador++;
        process.title = `BloodHound | Apagar package | ${contador}/${totalDMs} DMs limpas`
...
async function clearPackage() {
  console.clear();
  criarConfig();
  process.title = "BloodHound | Apagar package"

  // Verifica se está no Termux
  const isTermux = process.env.TERMUX === 'true' || process.env.PREFIX === '/data/data/com.termux/files/usr';
  
  if (isTermux) {
    // Para Termux, usa uma abordagem diferente
    console.log("Modo Termux detectado. Por favor, coloque o arquivo package.zip na mesma pasta do script.");
