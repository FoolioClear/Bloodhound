...
async function menu(client) {
  ...
  if (await checarUpdates()) {
    console.log(`        ${cor}[!]${reset} Há uma atualização disponível, vá em https://github.com/seu-usuario/bloodhound`);
  }
...
async function checarUpdates() {
  if ((await (await fetch("https://api.github.com/repos/seu-usuario/bloodhound/releases/latest")).json()).tag_name !== VERSAO_ATUAL) {
    return true;
  };
  return false;
}
